...
- `work/module-risk-scanner/` — 界面按 SukiSU 的结构重做：**悬浮底栏三个 tab**（主页 / 检查历史 / 设置）+ 横向 pager + 原有 `material-icons-extended` 图标 + **检查历史**（本地存档，最多 50 条）；版本 0.3.0/code4，主页与历史页分区、卡片比例已调整；**本机 Kotlin 编译通过**，等 CI 出包与真机看观感
