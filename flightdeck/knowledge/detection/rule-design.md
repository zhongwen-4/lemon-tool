...
## 例外：卸载脚本不参与升级判定（2026-09-21）

`uninstall.sh` 只在模块被卸载时执行。`scan.cpp` 的 `scan_text()` 对它：

- **照扫**：仍进文件清单、仍列在脚本清单里，特征表命中照记；
- **不跑升级表**，也不做 `obf.blob` / `obf.eval-dynamic` 这两条内联升级 —— 它的记录最高只到低危；
- 报告里加一条提示：「卸载脚本 uninstall.sh 只在模块被卸载时执行，其中的行为只作记录、不计入风险统计」。

理由：模块的卸载脚本删自己的 `/data/adb/...` 是**必然而正常**的收尾动作，把它算进「装不装」的风险
判定只会制造误报 —— 实测一个只剩自清理的良性模块会被判「中危 2」，结论从「未发现高危或中危行为」
掉成「有 2 项需要留意」。修完同一个样本是「高危 0 · 中危 0 · 低危 4」+ 一条提示。

反面：**别顺手把整个文件跳过**。它里面的低危记录（外部地址、超长 base64、下载执行）对「这个模块
像不像正经东西」仍有信息量，只是不该由它决定能不能装。
...
- 两个夹具都带 `uninstall.sh`：good 侧断言「照扫照列、不升级、有提示」，evil 侧在卸载脚本里塞了
  `setenforce 0` / `chmod 777 /system/bin/evil` / `curl|sh`，断言 `cmd.setenforce-off` **不出现**，
  而 `uninstall.sh:` 仍出现在发现列表里。
